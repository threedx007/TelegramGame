...
export default function GameCanvas({
  ...
  const update = useCallback((deltaTime: number) => {
    ...
    if (!updatedPlayer.grounded) {
      updatedPlayer.velocityY += 0.6 * deltaMultiplier; // gravity
    }
    ...
    for (const pit of currentPits) {
      if (updatedPlayer.x + updatedPlayer.width > pit.x && 
        ...
        const groundY = canvas.height - 120 - updatedPlayer.height;
    ...
    if (!onPlatform && !inPit) {
      const groundY = canvas.height - 120 - updatedPlayer.height; // 120px земли + высота игрока
      if (updatedPlayer.y >= groundY) {
